...
object HandLandmarkerStorage {
    var latestHandLandmarks: List<List<NormalizedLandmark>>? = null
    var latestHandedness: List<Category>? = null  // Stores "Left" or "Right"
}
...
class HandLandmarkerHelper(
    ...
    private fun returnLivestreamResult(
        ...
        // Store both hand landmarks and handedness
        if (result.landmarks().isNotEmpty()) {
            HandLandmarkerStorage.latestHandLandmarks = result.landmarks()
            HandLandmarkerStorage.latestHandedness = result.handedness().flatten()

            sendLandmarksToWebSocket(WebSocketClient)
        } else {
            HandLandmarkerStorage.latestHandLandmarks = null
            HandLandmarkerStorage.latestHandedness = null
        }
    ...
    private fun sendLandmarksToWebSocket(webSocketClient: WebSocketClient) {
        val detectedResults = HandLandmarkerStorage.latestHandLandmarks

        if (detectedResults.isNullOrEmpty()) {
            Toast.makeText(context, "No hands detected!", Toast.LENGTH_SHORT).show()
            return
        }
        ...
        // Send to WebSocketClient (uses built-in formatLandmarks function)
        webSocketClient.sendLandmarks(leftHand, rightHand)
    }
